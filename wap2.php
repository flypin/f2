...
if(!$content){
	echo '<p>Not any Data</p><p><a href="wap2.php">Home</a></p></body></html>';
	exit;
}
...
function ex($cs){
	...
	$text = preg_replace("(@([a-zA-Z0-9_]+))", '@<a href="wap2.php?uid=\\1">\\1</a>', $text);
		...
		foreach ($cs->entities->urls as $u) {
			...
			if (strpos($u->expanded_url, 'https://twitter.com/') !== false && strpos($u->expanded_url, '/status/') !== false) {
				...
				if (strpos($u->expanded_url, 'instagram.com/p/') !== false) {
					$text .= '<img src="'.$u->expanded_url.'media/?size=l" />';
				}
				if (strpos($u->expanded_url, 'pbs.twimg.com') !== false) {
					$text .= '<img src="'.$u->expanded_url.'" />';
				}
	...
	if(isset($cs->extended_entities->media)){
		...
		foreach ($cs->extended_entities->media as $m) {
			if($m->sizes->medium->w > 2048)
				$text .= '<a href="'.$m->media_url_https.':orig" target="_blank"><img src="'.$m->media_url_https.'" /></a>';
			elseif($m->sizes->large->w > 1020)
				$text .= '<a href="'.$m->media_url_https.':large" target="_blank"><img src="'.$m->media_url_https.'" /></a>';
			else
				$text .= '<img src="'.$m->media_url_https.'" />';
...
function status($cs,$q=0){
	...
	$username = '<a href="wap2.php?uid='.$cs->user->screen_name.'">'.$cs->user->screen_name.'</a>';
	$myText = ex($cs);
...
function page($sid,$mid){
	...
	$news .= '">news</a> ';
	$older .= '">older</a>';
	$temp = $older.' <a href="wap2.php">Home</a> '.$news;
	return $temp;
}
?>
</body>
